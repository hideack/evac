...
core.appendPluginPath = function(path) {
  pluginPaths.push(path);
};

core.parseRecipeJson = function(recipe, callback) {
  if (Array.isArray(recipe)) {
    recipe.forEach(function(oneRecipe) {
      core.parseRecipeJson(oneRecipe, callback);
    });
  ...
  pluginType.forEach(function(type) {
    ...
    for (var plugin in recipe[type]) {
      pluginPaths.forEach(function(path) {
        ...
        logger.info("Search plugin : " + pluginFileName);
    ...
    if (!plugins[type]) {
      callback(type + " plugin not found.", "Not defined.");
      return;
    }
  });
  ...
  try {
    plugins['in'].load(args['in'], function(err, outputs){
      ...
      var filtering = function() {
        var passedWord = outputs.shift();

        plugins['filter'].filter(args['filter'], passedWord, function(err, output){
          logger.info("Filter plugin received: " + passedWord);
          ...
          if (output == "") {
            ...
            logger.info("COMPLETE:\t" + passedWord + "\t" + output);
...
core.start = function(program, args) {
  ...
  if (program.path) {
    core.appendPluginPath(program.path);
  }
