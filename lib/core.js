...
var pluginPaths = [__dirname + '/plugin', '/Users/usr0600170/pj/private/evac/tmp'];
...
core.parseRecipeJson = function(recipe, callback) {
  ...
  pluginPaths.forEach(function(path) {
    for (var plugin in recipe.out) {
      var pluginFileName = path + '/out/' + plugin.toLowerCase() + '.js';
      outputPlugin = require(pluginFileName);
      outputArgs = recipe.out[plugin];
    }

    for (var plugin in recipe.filter) {
      var pluginFileName = path + '/filter/' + plugin.toLowerCase() + '.js';
